...
for data_dir in all_data_dirs:
	with open(os.path.join(data_dir,'driving_log.csv')) as csvfile:
		...
		next(reader) # skip first row
		for line in reader:
			img = os.path.join(data_dir, line[0].strip())
			img = cv2.imread(img)
			if use_left_right:
				...
				images.extend([img, img_left, img_right, \
							   np.fliplr(img), np.fliplr(img_left), np.fliplr(img_right)])
				steerings.extend([float(line[3]), float(line[3])+st_correction, float(line[3])-st_correction, \
								  -float(line[3]), -float(line[3])-st_correction, -float(line[3])+st_correction])
			else:
				images.extend([img, np.fliplr(img)])
				steerings.extend([float(line[3]), -float(line[3])])
		print("{}: {}".format(data_dir, len(images)))


img_shape = images[0].shape #(160,320,3)
X_train = np.array(images)
